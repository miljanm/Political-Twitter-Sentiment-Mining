...
def query_twitter(keywords):
    ...
    try:
        ...
        pickle.dump(all_tweets, open('./data/UK_debate_tweets_' + strftime("%Y-%m-%d_%H:%M:%S", gmtime()) + '.pickle',
                                     'wb'))
...
def clean_tweets(filename, remove_similar=True):
    ...
    tweets = pickle.load(open('./data/' + filename, 'rb'))
...
def write_tweets_to_file(tweets, filename):
    ...
    """
    with open('./data/' + filename, 'wb') as file1:
            file_writer = csv.writer(file1, dialect=csv.excel_tab)
            for key in tweets:
                file_writer.writerow(tweets[key])


if __name__ == '__main__':
    # query_twitter(['#BattleForNumber10'])
    # processed_tweets = clean_tweets('UK_debate_tweets.pickle', remove_similar=True)
    # write_tweets_to_file(processed_tweets, 'tweets_uk_debate.tsv')
    pass